...
const DSA_TOPICS_TIER = {
    critical: ['dynamic programming', 'arrays', 'trees', 'graphs', 'binary search', 'string'],
    important: ['greedy', 'linked lists', 'sorting', 'hash table', 'two pointers', 'math', 'number theory', 'stack'],
...
export function analyzeSkills(platformData) {
    ...
    if (lc?.tags) {
        lc.tags.forEach(t => {
            const key = normalizeTopic(t.tagName);
            const solvedCount = Number(t.problemsSolved) || 0;
            if (!key || solvedCount <= 0) return;
            allTags[key] = (allTags[key] || 0) + solvedCount;
        });
    }
    ...
    if (gfg?.stats?.totalSolved > 0) {
        ...
        allTags[normalizeTopic('arrays')] = (allTags[normalizeTopic('arrays')] || 0) + Math.floor(gfgTotal * 0.2);
        allTags[normalizeTopic('string')] = (allTags[normalizeTopic('string')] || 0) + Math.floor(gfgTotal * 0.1);
        allTags[normalizeTopic('trees')] = (allTags[normalizeTopic('trees')] || 0) + Math.floor(gfgTotal * 0.1);
        allTags[normalizeTopic('dynamic programming')] = (allTags[normalizeTopic('dynamic programming')] || 0) + Math.floor(gfgTotal * 0.08);
        allTags[normalizeTopic('sorting')] = (allTags[normalizeTopic('sorting')] || 0) + Math.floor(gfgTotal * 0.08);
        allTags[normalizeTopic('math')] = (allTags[normalizeTopic('math')] || 0) + Math.floor(gfgTotal * 0.07);
    }
    ...
    const hasTopicSignals = Object.keys(allTags).length > 0;

    if (hasTopicSignals) {
        DSA_TOPICS_TIER.critical.forEach(topic => {
            const normalizedTopic = normalizeTopic(topic);
            const count = allTags[normalizedTopic] || 0;

            if (count >= 15) {
                strongTopics.push({ topic: normalizedTopic, count, tier: 'critical' });
            } else if (count < 5) {
                weakTopics.push({ topic: normalizedTopic, count, tier: 'critical' });
                analysis.dsaImprovements.push({
                    topic: normalizedTopic.charAt(0).toUpperCase() + normalizedTopic.slice(1),
        ...
        DSA_TOPICS_TIER.important.forEach(topic => {
            ...
            if (count >= 10) {
                strongTopics.push({ topic: normalizedTopic, count, tier: 'important' });
            } else if (count < 3) {
                weakTopics.push({ topic: normalizedTopic, count, tier: 'important' });
                analysis.dsaImprovements.push({
                    topic: normalizedTopic.charAt(0).toUpperCase() + normalizedTopic.slice(1),
        ...
        analysis.weaknesses.push('Topic-wise breakdown is unavailable from connected platform data.');
        analysis.recommendations.push('Sync LeetCode again or log manual DSA topics to unlock accurate topic-level insights.');
    }

    // ========== COMPANY MATCHING ==========
    const userStrongTopics = new Set(strongTopics.map(t => normalizeTopic(t.topic)));
    const companyScores = [];
    ...
    for (const [company, topics] of Object.entries(COMPANY_DSA_MAP)) {
        ...
        topics.forEach((topic, idx) => {
            ...
            totalWeight += weight;
            if (userStrongTopics.has(normalizedTopic) || (allTags[normalizedTopic] || 0) >= 8) {
                matchCount += weight;
            }
        });
        ...
        if (score >= 30) {
            companyScores.push({
                ...
                matchedTopics: topics.filter((topic) => {
                    const normalizedTopic = normalizeTopic(topic);
                    return userStrongTopics.has(normalizedTopic) || (allTags[normalizedTopic] || 0) >= 8;
                }),
            });
        }
    }
